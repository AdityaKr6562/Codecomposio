...
<Step title="Create or get the OpenAPI Spec for the App">
  The first step is to get the OpenAPI Spec for the app you want to integrate
  with Composio. If the app does not have an OpenAPI Spec, you can create one
...
<Tip>
  ...
  it by searching "custom_tool_name auth docs" on Google. The first result often
  has what you need.
</Tip>
